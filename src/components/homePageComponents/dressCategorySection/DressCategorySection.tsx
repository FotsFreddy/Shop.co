...
export default function DressCategorySection() {
  ...
  return (
    <div className="bg-[#F0F0F0] rounded-3xl px-4 py-4 mx-4 my-10 md:p-10 md:mt-14 md:mx-12">
...
function DressStyle({item, colspan}:DressStyleProps) {
    return (
       <div className={`relative rounded-3xl overflow-hidden h-52 cursor-pointer md:h-64 ${colspan ? "md:col-span-2" : ""}`}>
        <Link to={"/Category"} aria-label={`Browse ${item.title} style`}>
            <img src={item.imageSrc} alt={item.title} className="w-full h-full  transition-all duration-300 object-cover hover:scale-110"/>
